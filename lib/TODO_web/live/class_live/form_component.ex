defmodule TODOWeb.ClassLive.FormComponent do
  ...
  def render(assigns) do
    ...
    <div>
      ...
      >
        ...
        <.input field={@form[:example]} type="text" label="Example" />
        <:actions>
  ...
  def update(%{class: class} = assigns, socket) do
    changeset = Tailwind.change_class(class)
  ...
  def handle_event("validate", %{"class" => class_params}, socket) do
    changeset =
      ...
      |> Tailwind.change_class(class_params)
  ...
  defp save_class(socket, :edit, class_params) do
    case Tailwind.update_class(socket.assigns.class, class_params) do
      {:ok, class} ->
  ...
  defp save_class(socket, :new, class_params) do
    case Tailwind.create_class(class_params) do
      {:ok, class} ->
        notify_parent({:saved, class})
